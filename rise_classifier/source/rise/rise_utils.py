...
class RiseUtils:
    ...
    @staticmethod
    def rules_accuracy(instances: List[Instance], rules: List[InstanceRule])->float:
        """
            calculate the accuracy of rules on the instances
        """
        predicted = 0
        for instance in instances:
            y_hat = RiseUtils.closest_rule(instance, rules)
            predicted += 1 if y_hat == instance.label else 0  
        return predicted/len(instances)

    @staticmethod
    def closest_rule(instance: Instance, rules: List[InstanceRule])->str:
        """
        ...
        for rule in rules:
            distance = rule.distance(instance)
            if distance < min:
                min = distance
                closest_rule = rule
    ...
    @staticmethod
    def closest_instance(rule: InstanceRule, instances: List[Instance]) -> Instance:
        """
